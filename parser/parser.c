...
/*   Updated: 2025/11/05 13:41:29 by fmoulin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_input_init(t_cmd **current_cmd, t_cmd **cmd_list, t_redir **redirection_list, int *i)
{
	*current_cmd = NULL;
    *cmd_list = NULL;
...
t_cmd	*parse_input(char *user_input, t_shell *shell)
{
	...
	while (tokens[i])
	{
		...
		if (handle_redir_case(tokens, &i, &current_cmd, &cmd_list))
			continue ;
		if (!current_cmd && !handle_command(tokens, &i, &cmd_list, &redirection_list))
