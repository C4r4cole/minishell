...
/*   Updated: 2025/09/24 15:38:28 by fmoulin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_quoting_symbols(char c)
{
	return (is_pipe(c) || is_slash(c) || is_hyphen(c) || is_underscore(c)
			...
			|| is_redirection_in(c) || is_redirection_out(c) || is_open_bracket(c)
			|| is_tilde(c) || is_percent(c) || is_ampersand(c) || is_semicolon(c)
			|| is_opening_parenthesis(c) || is_closing_parenthesis(c) || is_space(c)
